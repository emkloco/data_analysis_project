...
class HollowVisualizer:
    def run(self):
        ...
        # comparing 2002 (the good old days) vs 2022 (now)
        ...
        plt.title('Fig 2: The "Hollow Middle" - Disappearance of Affordable Markets', fontsize=16)
        plt.xlabel('Housing Cost Ratio (Price / Earnings)')
        plt.xlim(0, 20)
        plt.legend()
        
        plt.text(4, 0.15, "The Safe Middle\n(Gone)", color=config.COLOR_GOOD, ha='center')
        plt.text(12, 0.05, "The Crisis Tail\n(New Normal)", color=config.COLOR_BAD, ha='center')
